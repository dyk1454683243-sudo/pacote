...
class FetcherBase {
  ...
  #tarxOptions ({ cwd }) {
    ...
    return {
      ...
      noMtime: true,
      filter: (name, entry) => {
        // Skip symbolic links only. node-tar marks a regular file as type
        // 'Link' when nlink > 1 (shared inode / hardlink). Those entries
        // are file content and must be extracted. See npm/pacote#285.
        if (entry.type === 'SymbolicLink') {
          return false
        }
        // Hardlinks are not File$, but they should get the same mode bits.
        entry.mode = this.#entryMode(
          entry.path,
          entry.mode,
          entry.type === 'Link' ? 'File' : entry.type
        )
        // this replicates the npm pack behavior where .gitignore files
        // are treated like .npmignore files, but only if a .npmignore
        // file is not present.
        if (entry.type === 'Link' || /File$/.test(entry.type)) {
