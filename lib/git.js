...
const hostedTarballAuth = require('./util/hosted-tarball-auth.js')
  ...
  #clone (handler, tarballOk = true) {
    ...
    return cacache.tmp.withTmp(this.cache, o, async tmp => {
      ...
      if (tarballOk) {
        ...
        const tarballUrl = h.tarball({ noCommittish: false })
        // hosted-git-info tarball URLs (codeload/CDN) omit git userinfo, and
        // npm-registry-fetch looks up auth by the request host. Forward URL
        // credentials and git-host credential-store entries as forceAuth so
        // the CDN download gets an Authorization header.
        const forceAuth = hostedTarballAuth(h, tarballUrl, this.opts)
        return new RemoteFetcher(tarballUrl, {
          ...this.opts,
          ...(forceAuth ? { forceAuth } : {}),
          allowGitIgnore: true,
